...
export default function QueuePage() {
	...
	const { role: roleValue } = useAuthState();
	...
	const handleAddJob = async () => {
		if (!urls.trim()) return;
		try {
			const data = await addJobMutation.mutateAsync({
				urls,
						...
						<span
							className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
								roleValue === "admin"
									...
									: roleValue === "premium_member"
										? "bg-accent/10 text-accent border border-accent/20"
										: "bg-content-subtle/10 text-content-muted border border-border"
							}`}
						>
							{roleValue.replace("_member", "").replace("guest", "Guest")}
						</span>
					)}
				</div>
				<div className="flex items-center gap-3">
						...
						<div className="absolute bottom-3 left-4 flex flex-wrap gap-2 pointer-events-none">
							{urls.toLowerCase().includes("youtube") && (
								<span className="px-2 py-1 rounded-md bg-accent/10 text-accent text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
									<Video width={12} height={12} className="text-current" />{" "}
									YouTube
								</span>
							)}
							{urls.toLowerCase().includes("tiktok") && (
								<span className="px-2 py-1 rounded-md bg-accent/10 text-accent text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
									<Music width={12} height={12} className="text-current" />{" "}
									TikTok
								</span>
							)}
							{urls.toLowerCase().includes("instagram") && (
								<span className="px-2 py-1 rounded-md bg-accent/10 text-accent text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
									<Camera width={12} height={12} className="text-current" />{" "}
									Instagram
								</span>
							)}
						</div>
					...
					<div className="rounded-lg border border-orange-200 bg-orange-50 p-4 text-sm text-orange-800 animate-in slide-in-from-top-2 shadow-sm">
						...
						<ul className="space-y-2">
							{skipped.map((s, i) => (
								<li
									key={i}
									className="flex items-center justify-between gap-4 p-2 bg-white/50 rounded-md border border-orange-100"
								>
									<span className="truncate flex-1">
										<span className="font-mono text-[10px] opacity-70 block leading-none mb-1">
											{s.url}
										</span>
									...
									{s.filename && s.category && s.dateFolder && (
										<button
											onClick={(e) => {
												...
												triggerInvisibleDownload(
													getDownloadUrl(
														`data/${s.category}/${s.dateFolder}/${s.filename}`,
													),
												);
											}}
				...
				{retryError && (
					<div className="rounded-lg border border-accent/30 bg-accent/10 backdrop-blur-md p-4 text-sm text-red-400 animate-in slide-in-from-top-2 shadow-lg flex items-start gap-3">
						<HelpCircle
							...
							height={18}
							strokeWidth={2}
							className="shrink-0 mt-0.5 text-content-muted"
						/>
