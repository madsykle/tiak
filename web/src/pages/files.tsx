...
export default function FilesEnhanced() {
	...
	const [allFiles, setAllFiles] = useState<FileItem[]>(
		cachedResponse?.allFiles || [],
	);
	const [loading, setLoading] = useState(!cachedResponse);
	const [usage, setUsage] = useState<DiskUsage | null>(
		cachedResponse?.usage || null,
	);
	...
	const showFeedback = useCallback(
		(type: "success" | "error", text: string) => {
			...
			window.clearTimeout(
				(showFeedback as typeof showFeedback & { timeoutId?: number })
					.timeoutId,
			);
			(showFeedback as typeof showFeedback & { timeoutId?: number }).timeoutId =
				window.setTimeout(() => {
					setFeedback(null);
				}, 3000);
		},
	...
	const fetchFiles = useCallback(
		async (force = false) => {
			if (
				cachedResponse &&
				!force &&
				Date.now() - cachedResponse.timestamp < 30000
			) {
	...
	const handleBatchDelete = async () => {
		if (
			!confirm(
				`Delete ${selectedPaths.size} files? This action cannot be undone.`,
			)
		)
			return;
		...
		try {
			...
			if (result.errors.length > 0) {
				...
				showFeedback(
					"error",
					`Deleted ${result.deleted.length} file(s), but some deletions failed`,
				);
			} else {
			...
			if (cachedResponse) {
				cachedResponse.allFiles = cachedResponse.allFiles.filter(
					(f) => !deletedSet.has(f.path),
				);
			}
	...
	const handleBatchMove = async (targetCategory: string) => {
		...
		if (movedCount > 0) {
			...
			showFeedback(
				"success",
				`Moved ${movedCount} file(s) to ${targetCategory}`,
			);
		} else {
	...
	const navigatePreview = useCallback(
		(direction: "next" | "prev") => {
			...
			const currentIndex = sortedFilesList.findIndex(
	...
	const hasNextPreview =
		previewIndex >= 0 && previewIndex < sortedFilesList.length - 1;
							...
							<div className="mt-3 space-y-1.5 max-w-[280px]">
								...
								<div className="h-1.5 w-full bg-surface-strong rounded-full overflow-hidden border border-border/30">
									<div
										...
										style={{
											width: `${Math.min(100, (usage.totalSize / (50 * 1024 * 1024 * 1024)) * 100)}%`,
										}}
									/>
								</div>
							</div>
						...
						{paginatedFiles.map(({ date, items }) => (
							<FileDateSection
								...
								onToggleDateSelection={(dateItems) => {
									...
									const allSelected = datePaths.every((path) =>
