...
export async function getFileInfo(path: string): Promise<FileInfo> {
	const res = await fetchWithAuth(
		`${API_BASE}/files/info?path=${encodeURIComponent(path)}`,
	);
	if (!res.ok) {
...
export async function retryJob(id: string): Promise<RetryResponse> {
	...
	if (!res.ok) {
		...
		throw new Error(
			errorData.message || errorData.error || "Failed to retry job",
		);
	}
	return res.json();
}
...
export async function redownloadJob(id: string): Promise<RetryResponse> {
	...
	if (!res.ok) {
		const errorData = await res.json().catch(() => ({}));
		throw new Error(
			errorData.message || errorData.error || "Failed to redownload job",
		);
	}
	return res.json();
}
...
export async function runMaintenance(): Promise<void> {
	const res = await fetchWithAuth(`${API_BASE}/maintenance/fix-categories`, {
		method: "POST",
	});
...
export async function backfillMetadata(): Promise<void> {
	const res = await fetchWithAuth(`${API_BASE}/maintenance/backfill-metadata`, {
		method: "POST",
	});
