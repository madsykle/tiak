		...
		<div className="pt-6 border-t border-border-subtle">
			...
			<div className="space-y-4">
				{syncStatus.unsyncedCount > 0 && (
					<div
						className={`flex items-center gap-2 text-xs animate-in slide-in-from-top-2 transition-all duration-300 ${syncStatus.status === "running" ? "text-zinc-500 opacity-50" : "text-content-muted"}`}
					>
						<AlertTriangle
							width={14}
							height={14}
						...
						<span>
							<strong
								className={
									syncStatus.status === "running" ? "" : "text-foreground"
								}
							>
								{syncStatus.unsyncedCount} new file(s)
							</strong>{" "}
							waiting to sync.
						</span>
					</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div>
						<label
							htmlFor="syncDest"
							className="block text-xs font-medium text-content-muted uppercase tracking-wider mb-2"
						>
							Destination Path
						</label>
					...
					<div>
						...
						<div className="flex gap-2 p-1 bg-surface-subtle/30 rounded-lg border border-border-subtle">
							<button
								...
								className={`flex-1 py-1 px-3 rounded-md text-xs font-medium transition-all ${syncMode === "copy" ? "bg-background text-foreground shadow-sm" : "text-content-muted hover:text-foreground"}`}
							>
								Backup
							</button>
							<button
								onClick={() => onSyncModeChange("sync")}
								className={`flex-1 py-1 px-3 rounded-md text-xs font-medium transition-all ${syncMode === "sync" ? "bg-background text-foreground shadow-sm" : "text-content-muted hover:text-foreground"}`}
							>
								Mirror
							</button>
						</div>
				...
				<div className="rounded-xl bg-surface-subtle/30 border border-border-subtle p-4 space-y-3">
					<div className="flex justify-between items-center text-xs">
						<span className="text-content-muted uppercase font-bold tracking-widest">
							Last Run
						</span>
						...
						<details className="group">
							<summary className="cursor-pointer text-xs text-content-muted hover:text-foreground select-none flex items-center gap-1 transition-colors">
								<ChevronDown
									...
									height={12}
									strokeWidth={2}
									className="group-open:rotate-90 transition-transform text-current"
								/>
								View Activity Logs
							</summary>
